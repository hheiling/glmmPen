...
#' @export
fit_dat_glm = function(dat,  lambda0 = 0, lambda1 = 0, conv = 0.001, nMC = 1000,
  ...
  for(i in 1:maxitEM){
    ...
    # M Step
    XZ_long = cbind(X[rep(1:nrow(X), each = nrow(u)),-1], Znew[,]) 
    
    fitM = glm(formula = y[rep(1:nrow(X), each = nrow(u))] ~ XZ_long, 
               family = family) # weights = rep(1/nrow(u), times=nrow(u)*nrow(X))
    gc()
    coef = fitM$coefficients
    print(coef)
    c0 = c0 + (coef == 0)^2
