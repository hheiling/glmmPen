...
out1 = glmmPen(formula = y ~ X + (X | grp), family = "binomial", 
               optim_options = optimControl(nMC_start = 200, nMC_max = 1000, maxitEM = 10, fit_type=1), 
               tuning_options = lambdaControl())

out1B = glmmPen(formula = y ~ X + (X | grp), family = "binomial", 
               optim_options = optimControl(nMC_start = 200, nMC_max = 1000, maxitEM = 10, 
                                            covar = "independent", fit_type=2), 
               tuning_options = lambdaControl())
