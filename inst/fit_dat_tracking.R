# Original fit_dat code

fit.dat.original = function(dat,  lambda0_scad = 0, lambda1_scad = 0, conv = 0.001, nMC = 1000, family = "binomial", type = "hmmcov", trace = 0, initcov = NULL, glmIALconv = 1e-12, d = 2, group, alpha = 1, vartol = 0.1, nMC_max = 5000, returnMC = F, ufull = NULL, coeffull = NULL, gibbs = F, maxitEM = 100, pnonzerovar = 0, ufullinit = NULL){
  ...
  for(i in 1:maxitEM){
    ...
    # Next line added - "save"
    save(u, Z, group, d, J, file = "Znew_gen2_Data.RData")
    ...
    if(type == "hmmcov"){
      ...
      start_time <- Sys.time()
      fit0 = grpreg(Znew, y[rep(1:nrow(X), each = nrow(u))], group=covgroup, penalty="grMCP", family="binomial",lambda = lambda0, offset = X[rep(1:nrow(X), each = nrow(u)),] %*% 
                      matrix(coef[1:ncol(X)],ncol = 1), alpha = alpha, active = active0, initbeta = c(0,coef[-c(1:ncol(X))]))
      end_time <- Sys.time()
      ...
      start_time <- Sys.time()
      fit1 = grpreg(X[rep(1:nrow(X), each = nrow(u)),-1], y[rep(1:nrow(X), each = nrow(u))], group=1:(ncol(X)-1), penalty="grMCP", family="binomial",lambda = lambda1, offset = as.matrix(Znew %*% matrix(coef[-c(1:ncol(X))],ncol = 1)), alpha = alpha, active = active1, initbeta = coef[c(1:ncol(X))])
      end_time <- Sys.time()
      print(start_time - end_time)
      gc()
