...
// [[Rcpp::export]]
arma::vec grp_CD_XZ_D2_std(const arma::vec& y, const arma::mat& X, const arma::mat& Z,
  ...
  // Create M x H matrices recording the center and scale values to use in standardization
  arma::mat center_mat(M,H); center_mat.zeros();
  arma::mat scale_mat(M,H); scale_mat.zeros();
  
  for(m=0;m<M;m++){
    center_mat.row(m) = center;
    scale_mat.row(m) = scale;
  }
  ...
  for(k=0;k<d;k++){
    ...
    for(i=0;i<n_k;i++){
      Zki = Zk.row(i);
      A = kron(u, Zki) * J_q;
      A_std = (A - center_mat) / scale_mat;
      eta.col(ids(i)) = as_scalar(Xk.row(i) * beta.elem(fef_cols)) + A_std * beta.elem(ref_cols) + offset(ids(i));
  ...
  while((iter<maxit) & (converged==0)){
    ...
    for(k=0;k<d;k++){
      ...
      for(i=0;i<n_k;i++){
        Zki = Zk.row(i);
        A = kron(u, Zki) * J_q;
        Xj = A.cols(idxj - p); // Can just use A since not standardizing random intercept
        zetaj = zetaj + Xj.t() * resid.col(ids(i));
    ...
    for(j=(J_X+1); j<J_XZ; j++){
      ...
      for(k=0;k<d;k++){
        ...
        for(i=0;i<n_k;i++){
          ...
          A = kron(u, Zki) * J_q;
          A_std = (A - center_mat) / scale_mat;
          shift_ref = A_std.cols(idxr - p) * (beta.elem(idxr) - beta0.elem(idxr));
          // Update random effects component of eta for each individual
          eta.col(ids(i)) += shift_ref;
          // Update residuals
          resid.col(ids(i)) -+ shift_ref;
          // Calculate zetaj
    ...
    for(k=0;k<d;k++){
      ...
      for(i=0;i<n_k;i++){
        ...
        A_std = (A - center_mat) / scale_mat;
        // Update random effects component of eta for each individual
        eta.col(ids(i)) += A_std.cols(idxr - p) * (beta.elem(idxr) - beta0.elem(idxr));
      } // End i for loop
