...
// [[Rcpp::export]]
arma::vec grp_CD_XZ_B_ortho(const arma::vec& y, const arma::mat& X, const arma::mat& Z,
  ...
  if(H > q){
    ...
    for(f=0;f<q;f++){
      if(f == 0){
        ...
        ortho_factor[f] = emat.each_row() % trans(1/sqrt(evec));
      }
    }
  ...
  // Calculate residuals, determine initial nu and v0
  ...
  while((iter<maxit) & (converged==0)){
    ...
    for(k=0;k<d;k++){
      ...
      for(m=0;m<M;m++){
        m0 = m;
        arma::mat A = kron(u.submat(m0,col_idx),Zk) * J_q;
        // Do not need to calculate A_std or A_ortho since the columns related 
        // to the random intercept are not standardized (neither centered nor scaled) 
        // and they are not orthogonalized
        Xj = A.cols(idxj - p);
      ...
      for(k=0;k<d;k++){
        ...
        arma::mat Xj(ids.n_elem, Kj); Xj.zeros();
          ...
          if(H == q){ // sigma specified with independence structure (diagonal)
            ...
            // Standardization of A matrix sufficient
            ...
            for(m=0;m<M;m++){
              ...
              // Update random effects component of eta and residuals for each individual
            ...
            // Standardization and Orthogonalization of A matrix required
            ...
            for(m=0;m<M;m++){
              ...
              arma::rowvec shift = trans(A_ortho.cols(idxr - p) * (beta.elem(idxr) - beta0.elem(idxr)));
              // Update random effects component of eta and residuals for each individual
              eta.submat(m0,ids) += shift;
              resid.submat(m0,ids) -+ shift;
              // Calculate zetaj
