...
// Faster version of grp_CD_XZ: Don't update residuals after every update to the random effects
...
arma::vec grp_CD_XZ_step(const arma::vec& y, const arma::mat& X, const arma::mat& Z,
  ...
  arma::vec zeta_ref(H);
  arma::uvec idx_ref = find(XZ_group >= J_X); // index values for all random effects
  ...
  Q0 = (-1.0) * Qfun(y, X, Z, pBigMat, group, J_q, beta, offset, dims.subvec(2,3), family, link, sig_g, phi);
  ...
  while((iter<maxit) & (converged==0)){
    ...
    // Identify covariates belonging to random effects (group j >= J_X)
    arma::uvec idx_ref = find(XZ_group >= J_X); // index values for all random effects
    ...
    Kj = idxj.n_elem;
    arma::vec zetaj = zeta_ref(idxj - p);
    ...
    for(j=(J_X+1); j<J_XZ; j++){
      ...
      arma::vec zetaj = zeta_ref.elem(idxj - p);
    ...
    if((std::strcmp(family, bin) == 0) && (link == 10)){
      ...
      // 1. (a) Calculate Q-function using updated coefficients (see Qfun in "utility_glm.cpp")
      Q = (-1.0) * Qfun(y, X, Z, pBigMat, group, J_q, beta, offset, dims.subvec(2,3), family, link, sig_g, phi);
      // 1. (b) Calculate quadratic approximation to the Q-function based on Taylor series
      // expansion around the beta0 coefficients, see Qfun_quad_beta in "utility_grpCD.cpp"
